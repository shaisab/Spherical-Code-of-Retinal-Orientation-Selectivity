function []= generateEqualContributionAllCellsFile()
...
% allCells.XYZvecComp.X=[X;Opposite_X];
% allCells.XYZvecComp.Y=[Y;Opposite_Y];
% allCells.XYZvecComp.Z=[Z;Z];
% allCells.XYZvecComp.VecX=[VecX;Opposite_VecX];
% allCells.XYZvecComp.VecY=[VecY;Opposite_VecY];
% allCells.XYZvecComp.VecZ=[VecZ;VecZ];

% X=[X;X];
% Y=[Y;Y];
% Z=[Z;Z];
% VecX=[VecX;Opposite_VecX];
% VecY=[VecY;Opposite_VecY];
% VecZ=[VecZ;VecZ];
...
allCells.OSIBars=ones(2*size(X,1),1);
...
allCells.poriSz_bars=ones(2*size(X,1),1);
allCells.poriSz_bars_ON=ones(2*size(X,1),1);  
allCells.poriSz_bars_OFF=ones(2*size(X,1),1);
