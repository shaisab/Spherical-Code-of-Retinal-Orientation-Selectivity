function [allPossibleFields]= VecComparisonPooled4(data,a,maxVec,alpha,beta,polarity,fieldType,VizAngle,allPossibleFields,k,h,interpType,histType,DISTstate,locDist,widthDist)
...
switch histType
    case 5
        ...
        DISTstate=0;
%         % for setting an artificial map as the real data 
%         load('maxVec_Calcium imaging data_translation_25percentAll4ONOFF_40_DS_created_Jul_07_2015_15_06.mat');
%         VecX=maxVec.X';
%         VecY=maxVec.Y';
%         VecZ=maxVec.Z';
    ...
    case 1
        ...
        [alphaCanal,betaCanal]=AB(normal);
        alphaCanal=rad2deg(alphaCanal);
        betaCanal=rad2deg(betaCanal);
    ...
    case -1
        %%% observed vectors are set to the anti sense vector field around the ASC canal
        ...
        alphaCanal=rad2deg(alphaCanal);
        betaCanal=rad2deg(betaCanal);
        betaCanal=90-abs(90-betaCanal);
        if alphaCanal<=180
            alphaCanal=180+alphaCanal;
        else alphaCanal>180 
            alphaCanal=alphaCanal-180;
        end
...
%         VecZ=VecZ';
        ...
        %%% 16.7% of each of the six canalcentric channels
        f=randi(6,[size(data.XYZvecComp.VecXcompLSC,1),1]);
        X=[data.XYZvecComp.X(f==1);data.XYZvecComp.X(f==2);data.XYZvecComp.X(f==3);data.XYZvecComp.X(f==4);data.XYZvecComp.X(f==5);data.XYZvecComp.X(f==6)];
        Y=[data.XYZvecComp.Y(f==1);data.XYZvecComp.Y(f==2);data.XYZvecComp.Y(f==3);data.XYZvecComp.Y(f==4);data.XYZvecComp.Y(f==5);data.XYZvecComp.Y(f==6)];
        Z=[data.XYZvecComp.Z(f==1);data.XYZvecComp.Z(f==2);data.XYZvecComp.Z(f==3);data.XYZvecComp.Z(f==4);data.XYZvecComp.Z(f==5);data.XYZvecComp.Z(f==6)];
        X=X';
        Y=Y';
        Z=Z';
        
        VecX=[data.XYZvecComp.VecXcompASC(f==1);data.XYZvecComp.VecXcompPSC(f==2);data.XYZvecComp.VecXcompLSC(f==3);-data.XYZvecComp.VecXcompASC(f==4);-data.XYZvecComp.VecXcompPSC(f==5);-data.XYZvecComp.VecXcompLSC(f==6)];
        VecY=[data.XYZvecComp.VecYcompASC(f==1);data.XYZvecComp.VecYcompPSC(f==2);data.XYZvecComp.VecYcompLSC(f==3);-data.XYZvecComp.VecYcompASC(f==4);-data.XYZvecComp.VecYcompPSC(f==5);-data.XYZvecComp.VecYcompLSC(f==6)];
        VecZ=[data.XYZvecComp.VecZcompASC(f==1);data.XYZvecComp.VecZcompPSC(f==2);data.XYZvecComp.VecZcompLSC(f==3);-data.XYZvecComp.VecZcompASC(f==4);-data.XYZvecComp.VecZcompPSC(f==5);-data.XYZvecComp.VecZcompLSC(f==6)];
        VecX=VecX';
        VecY=VecY';
        VecZ=VecZ';
...
% calculate mean output based on tuning curve
    for m=1:size(angleXYZ,2)
        if ~isnan(angleXYZ(1,m))&& angleXYZ(1,m)>90
            ...
            output(m,1)=tuning.meanR(tuning.meanx==abs(floor(angleXYZ(1,m))-180)); % for new tuning curve
        elseif ~isnan(angleXYZ(1,m))&& angleXYZ(1,m)<=90
             %output(m,1)=1-tuning.meanR(tuning.meanx==floor(angleXYZ(1,m))); % for old tuning curve
            output(m,1)=tuning.meanR(tuning.meanx==floor(angleXYZ(1,m))); % for new tuning curve
        else
            output(m,1)=nan;
        end
    end
    countNan=sum(isnan(output));
...
%%%%%%%%% ALL THIS CODE IS FOR THE DISTANCE FUNCTION AND SHOULD BE APPLYED
%%%%%%%%% ON THE COMBINED MAP
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% %%% Apply distance function
% if DISTstate==0
%     scoreDist=ones(1,size(angleXYZ,2));
% elseif DISTstate==1
%     alphaCanal=deg2rad(alphaCanal);
%     betaCanal=deg2rad(betaCanal);
%     scoreDist=distanceFunction(alphaCanal,betaCanal,X,Y,Z,R,locDist,widthDist);
% end

% for c=10:10:10
%     cutoff=(['c',num2str(c)]);
%     matchind180.(cutoff)=100*(sum(scoreDist(find(angleXYZ<c)))/sum(scoreDist));
% end
% matchind90=[];
% angleXYZ90=[angleXYZ(find(angleXYZ<=90)),180-angleXYZ(find(angleXYZ>90))];
% %for c=5:5:45
% for c=10:10:10
%     cutoff=(['c',num2str(c)]);
% matchind90.(cutoff)=100*(numel(sum(scoreDist(angleXYZ90<c)))/sum(scoreDist));
% end
...
try allPossibleFields.countNan(k,h)=countNan; end
if k==1 && h==1
