function []= VecComparisonInterp(discPoints,fieldType,DSIlow,DSIhigh)
...
if sum(map.isDS)==size(map.isDS,1)      % true only for Hb9 anatomy cells
type=ones(size(map.isDS,1),1);   
else
type=map.isDS .* (map.DSI>DSIlow) .* (map.DSI<DSIhigh);      % include all DS cells with DSI between DSI low and DSI high in the analysis
end
n=size(Utemp,1);
...
for j=1:n,
    ...
    if type(j)==1
        ...
        try
            ...
            % comment when analyzing non DS results
...
try save(['vecComp_',num2str(discPoints),'_','DSI',num2str(DSIlow, '%0.2f'),'_',num2str(DSIhigh, '%0.2f'),'_',fieldType,'_',filename,'.mat'],'UVvecComp','STHvecComp','STHgenvecComp',...
            'XYZvecComp','UVStvecComp','ONorONOFForOFF','isRetro','injectionSite','isCART','isRBPMS','cellID','DSI','pResponse','onset1');
catch
save(['vecComp_',num2str(discPoints),'_','DSI',num2str(DSIlow, '%0.2f'),'_',num2str(DSIhigh, '%0.2f'),'_',fieldType,'_',filename,'.mat'],...
        'UVvecComp','STHvecComp','STHgenvecComp','XYZvecComp','UVStvecComp','pResponse','DSI'); 
end
% try save(['vecComp_',num2str(discPoints),'_','DSI',num2str(DSIlow, '%0.2f'),'_',num2str(DSIhigh, '%0.2f'),'_',fieldType,'_',filename,'.mat'],...
%         'UVvecComp','STHvecComp','STHgenvecComp','XYZvecComp','UVStvecComp','DSI'); end    
