function varargout = allCellsAnalysisGUIforBars(varargin)
% ALLCELLSANALYSISGUIFORBARS MATLAB code for allCellsAnalysisGUIforBars.fig
%      ALLCELLSANALYSISGUIFORBARS, by itself, creates a new ALLCELLSANALYSISGUIFORBARS or raises the existing
%      singleton*.
%
%      H = ALLCELLSANALYSISGUIFORBARS returns the handle to a new ALLCELLSANALYSISGUIFORBARS or the handle to
%      the existing singleton*.
%
%      ALLCELLSANALYSISGUIFORBARS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ALLCELLSANALYSISGUIFORBARS.M with the given input arguments.
%
%      ALLCELLSANALYSISGUIFORBARS('Property','Value',...) creates a new ALLCELLSANALYSISGUIFORBARS or raises the
%      existing singleton*.  Starting from the left, propecrty value pairs are
%      applied to the GUI before allCellsAnalysisGUIforBars_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to allCellsAnalysisGUIforBars_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help allCellsAnalysisGUIforBars

% Last Modified by GUIDE v2.5 04-Jan-2016 21:58:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @allCellsAnalysisGUIforBars_OpeningFcn, ...
                   'gui_OutputFcn',  @allCellsAnalysisGUIforBars_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before allCellsAnalysisGUIforBars is made visible.
function allCellsAnalysisGUIforBars_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to allCellsAnalysisGUIforBars (see VARARGIN)

% Choose default command line output for allCellsAnalysisGUIforBars
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes allCellsAnalysisGUIforBars wait for user response (see UIRESUME)
...
function varargout = allCellsAnalysisGUIforBars_OutputFcn(hObject, eventdata, handles) 
...
%         VectorCurveStandardTrans(fieldType,alphabar, beta,discPoints,plotType,graphcolor,VizAngle)
    ...
    try VectorCurveStandardTrans(fieldType,alphatrans(4)+alphashift,betatrans(4),discPoints,BGstate,'k',visAngle); end       %ventral
end
...
pooledmap=addClusterGridColor(pooledmap,COLORstate,f1,f2,f3);
...
switch ALPHACORRstate
    case 1
        ...
        namesASC=getFileList(filedir,['vecCompAlphaCorr_',num2str(discPoints),'_DSI0.10_1.00','_ASC'],0,'anywhere');    % DSI based on bars
        for k=1:size(namesASC,2)
            ...
            namesMap=getFileList(filedir,['vecCompAlphaCorr_',num2str(discPoints),'_DSI0.10_1.00'],0,'anywhere');       % DSI based on bars
...
plotAverageResponsePerType(ONorONOFForOFF,pResponse);
[idxT,inddataT]=clusteringDS5bars(pResponse,onset1,NORMALRESstate,postPcut);  
