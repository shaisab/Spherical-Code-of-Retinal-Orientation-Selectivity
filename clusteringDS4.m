function [idxT,inddataT,postmax]=clusteringDS4(pResponse,NORMALRESstate,postPcut,TYPECLUSTstate)
...
%% prepare data for PCA and sPCA analyses

if TYPECLUSTstate==2
    % sPCA analysis
    nSResponse=DSresponse;
    %nSResponse=SResponse./mean(SResponse,1);           % normalize responses to have mean zero
    nSResponse=nSResponse./vecnorm(nSResponse);        % to test that indeed the norm is 1 use: vecnorm(t)
    [B,SV,L,D,PATHS]=spca(nSResponse,[],10,inf,-500,50000)
    
    data=[SV(1).*B(:,1),SV(2).*B(:,2),SV(3).*B(:,3),SV(4).*B(:,4),SV(5).*B(:,5),SV(6).*B(:,6),...
        SV(7).*B(:,7),SV(8).*B(:,8),SV(9).*B(:,9),SV(10).*B(:,10)];
    
    %data=B;
    
    
elseif TYPECLUSTstate==1
    % PCA analysis
    
    %nSResponse=DSresponse;
   
    % % downsample to have the number of time points 50 times smaller than
    % the number of cells
    for d=1:size(DSresponse,2)
    nSResponse(:,d)=downsample(DSresponse(:,d),9);
    end
                    %     ricaA=rica(DSresponse,50);
                    %     nSResponse=ricaA.TransformWeights';
                    %     z=transform(ricaA,DSresponse)

    [coeff,score,latent,~,explained] = pca(nSResponse);
...
%         coeff(:,16),coeff(:,17),coeff(:,18),coeff(:,19),coeff(:,20)];

    data=[stdscore(1).*coeff(:,1),stdscore(2).*coeff(:,2),stdscore(3).*coeff(:,3),stdscore(4).*coeff(:,4),stdscore(5).*coeff(:,5),...
        stdscore(6).*coeff(:,6),stdscore(7).*coeff(:,7),stdscore(8).*coeff(:,8),stdscore(9).*coeff(:,9),stdscore(10).*coeff(:,10),stdscore(11).*coeff(:,11),...
        stdscore(12).*coeff(:,12),stdscore(13).*coeff(:,13),stdscore(14).*coeff(:,14),stdscore(15).*coeff(:,15),...
        stdscore(16).*coeff(:,16),stdscore(17).*coeff(:,17),stdscore(18).*coeff(:,18),stdscore(19).*coeff(:,19),stdscore(20).*coeff(:,20)];
...
for nK=2:1:10
    ...
    %GM=fitgmdist(data,nK,'RegularizationValue',0.001,'Replicates',50,'SharedCovariance',true,'Options',options)
    GM=fitgmdist(data,nK,'RegularizationValue',0.001,'Replicates',50,'CovarianceType','diagonal','Options',options)
...
gradientBIC=diff(-1*gradient(allBIC));
diffBIC=diff(allBIC);
...
% [~,OptimalK]=max(gradientBIC(1:end));
% [pks,locs]=findpeaks(gradientBIC);
% OptimalK=locs(1);
[~,OptimalK]=max(weightDeltaBIC);

% figure
% hold on
% plot(2:15,allBIC(2:15),'-ob');
% plot(2:15,deltaBIC(2:15),'-ob');
% plot(2:14,diff_allBIC(2:14),'-ob');
% plot(2:15,gradient_allBIC(2:15),'-ob');

% recalculate the Gaussian mixture models with the optimal number of clusters (optimalK)
%GM=fitgmdist(data,OptimalK,'RegularizationValue',0.001,'Replicates',50,'Options',options)
%GM=fitgmdist(data,nK,'RegularizationValue',0.001,'Replicates',50,'SharedCovariance',true,'Options',options)
GM=fitgmdist(data,OptimalK,'RegularizationValue',0.001,'Replicates',50,'CovarianceType','diagonal','Options',options)
idx=cluster(GM,data);
...
xlim([0 size(DSnorpResponse,1)]);
subplot(2,15,11);
plot(DSnorpResponse(:,idx==11),'k');
xlim([0 size(DSnorpResponse,1)]);
subplot(2,15,12);
plot(DSnorpResponse(:,idx==12),'k');
xlim([0 size(DSnorpResponse,1)]);
subplot(2,15,13);
plot(DSnorpResponse(:,idx==13),'k');
xlim([0 size(DSnorpResponse,1)]);
subplot(2,15,14);
plot(DSnorpResponse(:,idx==14),'k');
xlim([0 size(DSnorpResponse,1)]);
subplot(2,15,15);
plot(DSnorpResponse(:,idx==15),'k');
xlim([0 size(DSnorpResponse,1)]);
...
xlim([0 size(DSnorpResponse,1)]); ylim([0 1]);
subplot(2,15,26);
shadedErrorBar(1:size(DSnorpResponse,1),mean(DSnorpResponse(:,idx==11),2),std(DSnorpResponse(:,idx==11),[],2),'k');
xlim([0 size(DSnorpResponse,1)]); ylim([0 1]);
subplot(2,15,27);
shadedErrorBar(1:size(DSnorpResponse,1),mean(DSnorpResponse(:,idx==12),2),std(DSnorpResponse(:,idx==12),[],2),'k');
xlim([0 size(DSnorpResponse,1)]); ylim([0 1]);
subplot(2,15,28);
shadedErrorBar(1:size(DSnorpResponse,1),mean(DSnorpResponse(:,idx==13),2),std(DSnorpResponse(:,idx==13),[],2),'k');
xlim([0 size(DSnorpResponse,1)]); ylim([0 1]);
subplot(2,15,29);
shadedErrorBar(1:size(DSnorpResponse,1),mean(DSnorpResponse(:,idx==14),2),std(DSnorpResponse(:,idx==14),[],2),'k');
xlim([0 size(DSnorpResponse,1)]); ylim([0 1]);
subplot(2,15,30);
shadedErrorBar(1:size(DSnorpResponse,1),mean(DSnorpResponse(:,idx==15),2),std(DSnorpResponse(:,idx==15),[],2),'k');
xlim([0 size(DSnorpResponse,1)]); ylim([0 1]);
...
try figure;scatter(1:length(post(idx==4,4)),post(idx==4,4),'b'); end
...
try figure;scatter(1:length(post(idx==11,11)),post(idx==11,11),'b'); end
try figure;scatter(1:length(post(idx==12,12)),post(idx==12,12),'b'); end
try figure;scatter(1:length(post(idx==13,13)),post(idx==13,13),'b'); end
try figure;scatter(1:length(post(idx==14,14)),post(idx==14,14),'b'); end
try figure;scatter(1:length(post(idx==15,15)),post(idx==15,15),'b'); end
...
try indPost4=intersect(find(idx==4),find(post(:,4)<postPcut)); end
...
try indPost11=intersect(find(idx==11),find(post(:,11)<postPcut)); end
try indPost12=intersect(find(idx==12),find(post(:,12)<postPcut)); end
try indPost13=intersect(find(idx==13),find(post(:,13)<postPcut)); end
try indPost14=intersect(find(idx==14),find(post(:,14)<postPcut)); end
try indPost15=intersect(find(idx==15),find(post(:,15)<postPcut)); end




try combIndPost=[indPost1;indPost2;indPost3];end
try combIndPost=[indPost1;indPost2;indPost3;indPost4];end
try combIndPost=[indPost1;indPost2;indPost3;indPost4;indPost5];end
...
set(f12,'position',[5 150 1300 400]);

subplot(1,15,1,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,2,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,3,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,4,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,5,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,6,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,7,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,8,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,9,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,10,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
...
subplot(1,15,11,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
shadedErrorBar(time,mean(DSnorpResponseT(:,idxT==11),2),std(DSnorpResponseT(:,idxT==11),[],2),'k');
hold on
plot(time,mean(DSnorpResponseT(:,idxT==11),2),'b','LineWidth',1);
xlim([0 time(end)]); ylim([0 1]);
xlabel('Time (sec)','FontSize',16);
title(['\itn\rm = ',num2str(sum(idxT==11))],'FontSize',16);
box off

subplot(1,15,12,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
shadedErrorBar(time,mean(DSnorpResponseT(:,idxT==12),2),std(DSnorpResponseT(:,idxT==12),[],2),'k');
hold on
plot(time,mean(DSnorpResponseT(:,idxT==12),2),'b','LineWidth',1);
xlim([0 time(end)]); ylim([0 1]);
xlabel('Time (sec)','FontSize',16);
title(['\itn\rm = ',num2str(sum(idxT==12))],'FontSize',16);
box off

subplot(1,15,13,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
shadedErrorBar(time,mean(DSnorpResponseT(:,idxT==13),2),std(DSnorpResponseT(:,idxT==13),[],2),'k');
hold on
plot(time,mean(DSnorpResponseT(:,idxT==13),2),'b','LineWidth',1);
xlim([0 time(end)]); ylim([0 1]);
xlabel('Time (sec)','FontSize',16);
title(['\itn\rm = ',num2str(sum(idxT==13))],'FontSize',16);
box off

subplot(1,15,14,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
shadedErrorBar(time,mean(DSnorpResponseT(:,idxT==14),2),std(DSnorpResponseT(:,idxT==14),[],2),'k');
hold on
plot(time,mean(DSnorpResponseT(:,idxT==14),2),'b','LineWidth',1);
xlim([0 time(end)]); ylim([0 1]);
xlabel('Time (sec)','FontSize',16);
title(['\itn\rm = ',num2str(sum(idxT==14))],'FontSize',16);
box off

subplot(1,15,15,'Parent',f12,'YColor',[1 1 1],'XColor',[1 1 1]);
shadedErrorBar(time,mean(DSnorpResponseT(:,idxT==15),2),std(DSnorpResponseT(:,idxT==15),[],2),'k');
hold on
plot(time,mean(DSnorpResponseT(:,idxT==15),2),'b','LineWidth',1);
xlim([0 time(end)]); ylim([0 1]);
xlabel('Time (sec)','FontSize',16);
title(['\itn\rm = ',num2str(sum(idxT==15))],'FontSize',16);
box off
