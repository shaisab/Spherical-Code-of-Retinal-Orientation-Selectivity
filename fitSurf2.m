    function []= fitSurf2(channelID,resolution,mapType)
%% import data
...
%             order(k)=str2num(FileName{k}(indUnder(6)+1:indUnder(7)-1));     % for individual retinas
    ...
    case 2      % mixed vertical translation + horizontal rotation (singularities basde on real data)
        ...
        for i=1:size(nameSorted,2)
            if strfind(nameSorted{i},'translation_alpha')                               % two vertical translation channels
                ...
                transn=transn+1;
            elseif strfind(nameSorted{i},'rotation_alpha')      % two horizontal rotation channels
                load(num2str(nameSorted{i}));
    ...
    case 1          % rotation
        ...
        switch channelID
            case 1      % sense polarity for each of three canalcentric channels (3 channels) -  canal-based singularities
                ...
                SST=Y'*Y;
                R2=SSR/SST;
                R2_adjusted=1-((1-R2).*((length(Cr)-1)/(length(Cr)-2)));
                ...
                RMSE = sqrt(mean((Y - Cfit).^2));  % Root Mean Squared Error
...
%                 regressor=[C1,C2];
                ...
                %%% fit two translation channels 29/08/22
                % non linear optimization
%                 guess=[0.5,0.5];
%                 lowerBound=[0,0];
%                 upperBound=[1,1];
...
%                 %%% fit one translation channels 29/08/22
%                 % non linear optimization
                guess=0.5;
                lowerBound=0;
                upperBound=1;
...
%                 % non linear optimization
            ...
            case 4          % rotation channels based on real singularities
...
%         Error= rmse(Cfit,Y);
    ...
    case 2          % mixed vertical translation + horizontal rotation
        ...
        % reconstruct the contour using the estimated coefficients
        Cfit=x(1).*C1+x(2).*C2+x(3).*C3+x(4).*C4;
        CfitRs=reshape(Cfit,size(real,1),[]);
        CresRs=reshape(residual,size(real,1),[]);
        ...
        switch channelID
            case 1              % rotation
                title(['model  ','ASC:',num2str(xn(1),2),...
                    ',  PSC:',num2str(xn(2),2),',  LSC:',num2str(xn(3),2),',  R^2: ',num2str(R2_adjusted,2)]);
            case 2
%                 title(['model  ','ASC:',num2str(xn(1),2),...
%                     ',  PSC:',num2str(xn(2),2),',  LSC:',num2str(xn(3),2),'  iASC:',num2str(xn(4),2),...
%                     ',  iPSC:',num2str(xn(5),2),'  iLSC:',num2str(xn(6),2),',  R^2: ',num2str(R2,2)]);
                try title(['model  ','Dorsoventral: ',num2str(xn(1),2),...
            ',  Temporonasal: ',num2str(0),',  R^2: ',num2str(R2_adjusted,2)]); end
    
            case 3
%                 title(['model  ','ASC:',num2str(xn(1),2),...
%                     ',  LSC:',num2str(xn(2),2),'  iASC:',num2str(xn(3),2),...
%                     '  iLSC:',num2str(xn(4),2),',  R^2: ',num2str(R2,2)]);
                try title(['model  ','Dorsoventral: ',num2str(0),...
            ',  Temporonasal: ',num2str(xn(1),2),',  R^2: ',num2str(R2_adjusted,2)]); end
            case 4
                try title(['model  ','Dorsoventral: ',num2str(xn(1),2),...
            ',  Temporonasal: ',num2str(xn(2),2),',  R^2: ',num2str(R2_adjusted,2)]); end
        hold on;
        scatter(alphasing,betasing,'bo','LineWidth',3);
        scatter(alphasingDS,betasingDS,'kx','LineWidth',3);
        hold off;
...
%         scatter(alphaiLSC,betaiLSC,'ro','LineWidth',3);
    ...
    case 0              % translation
        switch channelID
            case 2        
                try title(['model  ','Temporonasal: ',num2str(0),...
            ', Dorsoventral: ',num2str(xn(1),2),',  R^2: ',num2str(R2_adjusted,2)]);end
    
            case 3        
                try title(['model  ','Temporonasal: ',num2str(xn(1),2),...
            ', Dorsoventral: ',num2str(0),',  R^2: ',num2str(R2_adjusted,2)]);end
                
            case 4
                try title(['model  ','Temporonasal: ',num2str(xn(2),2),...
            ', Dorsoventral: ',num2str(xn(1),2),',  R^2: ',num2str(R2_adjusted,2)]);end
        hold on;
        scatter(alphasing,betasing,'bo','LineWidth',3);
        scatter(alphasingDS,betasingDS,'kx','LineWidth',3);
    ...
    case 2              % mixed translation-rotation
        
        title(['model  ','trans field 1: ',num2str(xn(1),2),...
            ',  trans field 2: ',num2str(xn(2),2),',  rot field 1: ',num2str(xn(3),2),...
            ',  rot field 2: ',num2str(xn(4),2),',  R^2: ',num2str(R2_adjusted,2)]);
        hold on;
        scatter(alphasing,betasing,'bo','LineWidth',3);
        scatter(alphasingDS,betasingDS,'kx','LineWidth',3);
        hold off;
...
%         scatter(alphaLSC,betaLSC,'rx','LineWidth',3);
%         scatter(alphaiLSC,betaiLSC,'ro','LineWidth',3);
        scatter(alphasing,betasing,'bo','LineWidth',3); %29/08/22 daniel
        scatter(alphasingDS,betasingDS,'kx','LineWidth',3);
    case 0          % translation
        scatter(alphasing,betasing,'bo','LineWidth',3);
        scatter(alphasingDS,betasingDS,'kx','LineWidth',3);
...
%                 xlim([0.5 6.5]);
                 bar([xn(1);0],0.8);
                 ax.XTickLabel={'Dorsoventral','Temporonasal'};              
                 xlim([0.65 2.4]);
            case 3
...
%                 xlim([0.5 4.5]);
                 ...
                 xlim([0.65 2.4]);
            case 4
%             bar([xn(1),xn(2)],'k');     %for ON-OFF rotation fit where the alpha of the nasal axis is 350 deg
%             bar([xn(2),xn(1)],'k');     %for ON rotation fit where the alpha of the nasal axis is 5 deg
              bar([xn(1);xn(2)],0.8);
    ...
    case 0          % translation
        switch channelID
            case 2
                        bar([0;xn(1)],0.8);
        ax.XTickLabel={'Temporonasal','Dorsoventral'};
        xlim([0.65 2.4]);
