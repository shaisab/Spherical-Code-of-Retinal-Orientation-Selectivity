function [allPossibleFields]= VecComparisonPooled4Boot2(data,a,maxVec,alpha,beta,polarity,fieldType,VizAngle,allPossibleFields,k,h,interpType,histType,DISTstate,locDist,widthDist)
...
for i=1:Npooled
        [t(i),phi(i)]=InverseSTH(alpha,beta,R,SV(i),THV(i)); 
            ...
            vecSV(i)=-cos(SV(i)/R)*cos(beta)*sin(phi(i))*cos(THV(i)-alpha)+...
                cos(SV(i)/R)*cos(phi(i))*(sin(THV(i)-alpha))-...
                sin(SV(i)/R)*sin(phi(i))*sin(beta);
            
            vecSV(i)=R*sin(t(i)/R)*vecSV(i);
            
            vecTHV(i)=(cos(beta)*sin(phi(i))*sin(THV(i)-alpha)+...
                cos(phi(i))*cos(alpha-THV(i)));
        
            vecTHV(i)=R*sin(t(i)/R)*vecTHV(i);
            vecTHV(i)=1/(R*sin(SV(i)/R))*vecTHV(i);
