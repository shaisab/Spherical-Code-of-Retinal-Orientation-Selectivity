...
if ~isempty(names)
    for ncell=1:size(names,2)
        ...
        if strcmp(curname(inds(end)+1:length(curname)-4),'ONDS')
            ...
            onset1ONDS(:,ncell)=ONDSres.onset1;
        ...
        elseif strcmp(curname(inds(end)+1:length(curname)-4),'ONOFFDS')
            ...
            onset1ONOFFDS(:,ncell)=ONOFFDSres.onset1;
        end
...
onset1ONDS(:,~any(onset1ONDS,1))=[];  %columns
...
onset1ONOFFDS(:,~any(onset1ONOFFDS,1))=[];  %columns
...
peakLocONmaxfSec=TimeONDS(peakLocONmaxf);              % convert DSpeakLocOn from number of data points to time
peakLocONmaxfSec=peakLocONmaxfSec-onset1ONDS;    % adjust DSpeakLocOn relative to the onset of the stimulus

peakLocONOFFmaxfSec=TimeONOFFDS(peakLocONOFFmaxf);
peakLocONOFFmaxfSec=peakLocONOFFmaxfSec-onset1ONOFFDS;    % adjust DSpeakLocOn relative to the onset of the stimulus
...
summary.onset1ONDS=onset1ONDS;
summary.onset1ONOFFDS=onset1ONOFFDS;
