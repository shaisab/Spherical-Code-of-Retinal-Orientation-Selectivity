function varargout = allCellsAnalysisGUIHb9(varargin)
% ALLCELLSANALYSISGUIHB9 MATLAB code for allCellsAnalysisGUIHb9.fig
%      ALLCELLSANALYSISGUIHB9, by itself, creates a new ALLCELLSANALYSISGUIHB9 or raises the existing
%      singleton*.
%
%      H = ALLCELLSANALYSISGUIHB9 returns the handle to a new ALLCELLSANALYSISGUIHB9 or the handle to
%      the existing singleton*.
%
%      ALLCELLSANALYSISGUIHB9('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ALLCELLSANALYSISGUIHB9.M with the given input arguments.
%
%      ALLCELLSANALYSISGUIHB9('Property','Value',...) creates a new ALLCELLSANALYSISGUIHB9 or raises the
%      existing singleton*.  Starting from the left, propecrty value pairs are
%      applied to the GUI before allCellsAnalysisGUIHb9_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to allCellsAnalysisGUIHb9_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help allCellsAnalysisGUIHb9

% Last Modified by GUIDE v2.5 29-Oct-2015 10:48:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @allCellsAnalysisGUIHb9_OpeningFcn, ...
                   'gui_OutputFcn',  @allCellsAnalysisGUIHb9_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before allCellsAnalysisGUIHb9 is made visible.
function allCellsAnalysisGUIHb9_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to allCellsAnalysisGUIHb9 (see VARARGIN)

% Choose default command line output for allCellsAnalysisGUIHb9
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes allCellsAnalysisGUIHb9 wait for user response (see UIRESUME)
...
function varargout = allCellsAnalysisGUIHb9_OutputFcn(hObject, eventdata, handles) 
...
%         VectorCurveStandardTrans(fieldType,alphabar, beta,discPoints,plotType,graphcolor,VizAngle)
    try VectorCurveStandardTrans(fieldType,alphatrans(1)+alphashift,betatrans(1),discPoints,BGstate,'c',visAngle); end       %nasal     
    try VectorCurveStandardTrans(fieldType,alphatrans(2)+alphashift,betatrans(2),discPoints,BGstate,'c',visAngle); end       %dorsal  
    try VectorCurveStandardTrans(fieldType,alphatrans(3)+alphashift,betatrans(3),discPoints,BGstate,'c',visAngle); end       %temporal   
    try VectorCurveStandardTrans(fieldType,alphatrans(4)+alphashift,betatrans(4),discPoints,BGstate,'c',visAngle); end       %ventral
end
...
elseif COLORstate==3
    for Hb=1:size(pooledmap.UVStvecComp.USt,1) 
        set(0,'CurrentFigure', f1) %make the UVstandard figure active to plot the vectors
        % for morphology
        quiver(pooledmap.UVStvecComp.USt(Hb),pooledmap.UVStvecComp.VSt(Hb),pooledmap.UVStvecComp.VecUSt(Hb),pooledmap.UVStvecComp.VecVSt(Hb),1.2*pooledmap.DSI(Hb),'k','LineWidth',1,'MaxHeadSize',1);
        % for ephys
        %quiver(pooledmap.UVStvecComp.USt(Hb),pooledmap.UVStvecComp.VSt(Hb),pooledmap.UVStvecComp.VecUSt(Hb),pooledmap.UVStvecComp.VecVSt(Hb),0.1,'k','LineWidth',1,'MaxHeadSize',1);
    
        set(0,'CurrentFigure', f5) %make the spherical retina figure active to plot the vectors
        % for morphology
        quiver3(pooledmap.XYZvecComp.X(Hb),pooledmap.XYZvecComp.Y(Hb),pooledmap.XYZvecComp.Z(Hb),pooledmap.XYZvecComp.VecX(Hb),pooledmap.XYZvecComp.VecY(Hb),pooledmap.XYZvecComp.VecZ(Hb)+0.1,1*pooledmap.DSI(Hb),'k','LineWidth',1,'MaxHeadSize',1);
        % for ephys
        %quiver3(1.05.*pooledmap.XYZvecComp.X(Hb),1.05.*pooledmap.XYZvecComp.Y(Hb),1.05.*pooledmap.XYZvecComp.Z(Hb),1.05.*pooledmap.XYZvecComp.VecX(Hb),1.05.*pooledmap.XYZvecComp.VecY(Hb),1.05.*pooledmap.XYZvecComp.VecZ(Hb),0.1,'k','LineWidth',1.5,'MaxHeadSize',1);
    end
...
calculateAllPossibleVecFields(ALPHACORRstate,DATAMODELstate,CLUSTNstate,histType,FileName,resolution,mapType,DISTstate,locDist,widthDist,ABstate);
...
switch mapType
    case 1              % rotation
        ...
        alpha=alpharot(5);
        beta=betarot(5);
        calculateSingleChannelMap(ALPHACORRstate,DATAMODELstate,CLUSTNstate,histType,FileName,resolution,polarity2,addNoise,mapType,fieldName,alpha,beta)
...
switch polarity2
    case 1
        ...
        beta=rad2deg(betarot);
        calculateSingleChannelMap(ALPHACORRstate,DATAMODELstate,CLUSTNstate,histType,FileName,resolution,polarity2,addNoise,fieldType,fieldName,alpha,beta)
