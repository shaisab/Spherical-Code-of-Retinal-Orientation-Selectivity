function [] = VectorCurveStandardForExample(Normal,discPoints,plotType)
...
InRange1=sv1>=RV*pi/2;
InRange2=sv2>=RV*pi/2;
InRange3=sv3>=RV*pi/2;
InRange4=sv4>=RV*pi/2;
InRange5=sv5>=RV*pi/2;
InRange6=sv6>=RV*pi/2;
InRange7=sv7>=RV*pi/2;
InRange8=sv8>=RV*pi/2;
InRange9=sv9>=RV*pi/2;
InRange10=sv10>=RV*pi/2;
...
sret1=-2*M/pi*acos(-C1(3,:)/RV)+2*M;
thret1=atan2(C1(2,:),C1(1,:))+pi;
sret2=-2*M/pi*acos(-C2(3,:)/RV)+2*M;
thret2=atan2(C2(2,:),C2(1,:))+pi;
sret3=-2*M/pi*acos(-C3(3,:)/RV)+2*M;
thret3=atan2(C3(2,:),C3(1,:))+pi;
sret4=-2*M/pi*acos(-C4(3,:)/RV)+2*M;
thret4=atan2(C4(2,:),C4(1,:))+pi;
sret5=-2*M/pi*acos(-C5(3,:)/RV)+2*M;
thret5=atan2(C5(2,:),C5(1,:))+pi;
sret6=-2*M/pi*acos(-C6(3,:)/RV)+2*M;
thret6=atan2(C6(2,:),C6(1,:))+pi;
sret7=-2*M/pi*acos(-C7(3,:)/RV)+2*M;
thret7=atan2(C7(2,:),C7(1,:))+pi;
sret8=-2*M/pi*acos(-C8(3,:)/RV)+2*M;
thret8=atan2(C8(2,:),C8(1,:))+pi;
sret9=-2*M/pi*acos(-C9(3,:)/RV)+2*M;
thret9=atan2(C9(2,:),C9(1,:))+pi;
sret10=-2*M/pi*acos(-C10(3,:)/RV)+2*M;
...
figure('position',[450 150 950 800]);
hold on;

%Plotting Flattened Retina
    surf(StU1,StV1,zeros(ntemp,ntemp),'FaceColor','black','EdgeColor','none');
    surf(StU2,StV2,zeros(ntemp,ntemp),'FaceColor','black','EdgeColor','none');
    surf(StU3,StV3,zeros(ntemp,ntemp),'FaceColor','black','EdgeColor','none');
    surf(StU4,StV4,zeros(ntemp,ntemp),'FaceColor','black','EdgeColor','none');
    alpha(.20);
