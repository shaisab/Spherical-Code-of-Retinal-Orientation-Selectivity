...
function []= mapCellsAlphaCorrDSbyBars()
...
elseif size(metaFileList,2)==0 && size(mergedmetaFileList,2)>0
    ...
    m=sqrt(h^2+h^2)/2;  % half diagonal of the screen 
        ...
        for ROIn=1:size(mergedmeta.metadataDG{1}.ROIdata,2)
            ...
            pdir=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.pdir; % to be use when combining data from several retinas.
            pdirSz=pdir+A;
            if pdirSz<0
                pdirSz=360+pdirSz;
            end
            
            pori=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.pori; % to be use when combining data from several retinas.
            poriSz=pori+A;
            if poriSz<0
                poriSz=360+poriSz;
            end
            ...
            map.DSI(j,1)=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.DSI;
            
            if mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.DSI>=0.05
                map.isDS(j,1)=1;
            else
                map.isDS(j,1)=0;
            end
            ...
            map.DSI(j,1)=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.DSI;
            map.OSI(j,1)=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.OSI;
            try
                map.globalOSI(j,1)=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.globalOSI;
                map.oriFWHM(j,1)=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.oriFWHM;
                map.oriR2(j,1)=mergedmeta.meanRepDB.ROIdata{1,ROIn}.summary.oriR2;
            end
...
%         end
 ...
 % plot DS and OS cells and vectors
                ...
                plot(xctrans(ONDS),yctrans(ONDS),'o','MarkerSize',2,'MarkerEdgeColor','k');
                %text(xctrans(ONDS),yctrans(ONDS),[repmat('  ',size(xctrans(ONDS)),1),num2str(find(ONDS==1))],'HorizontalAlignment','left','Color','r');
                plot(xctrans(ONOFFDS),yctrans(ONOFFDS),'o','MarkerSize',2,'MarkerEdgeColor','k');
                ...
                scale=0.2;
                quiver(xctrans(ONDS),yctrans(ONDS),imadir(ONDS),imbdir(ONDS),scale,'k','linewidth',1);
                quiver(xctrans(ONOFFDS),yctrans(ONOFFDS),imadir(ONOFFDS),imbdir(ONOFFDS),scale,'k','linewidth',1);
