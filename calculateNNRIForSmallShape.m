function nnriDistribution = calculateNNRIForSmallShape(smallShape, numPoints, numIterations)
    ...
    % Array to store NNRI results from each iteration
    nnriResults = NaN(numIterations, 1);  % Initialize with NaN to track invalid cases
    ...
    nnriResults = nnriResults(~isnan(nnriResults));
    
    % Return the NNRI distribution
    nnriDistribution = nnriResults;
end
...
function pointsInside = pointsInsideTransformedShape(shape, points)
    ...
    shapeMask = poly2mask(shape.Vertices(:, 1), shape.Vertices(:, 2), 100, 100);
    
    % Find points that are inside the shape
    [rows, cols] = find(shapeMask);
    validPoints = [cols, rows];
    
    % Check if each point is inside the shape mask
