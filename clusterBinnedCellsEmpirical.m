...
function [] = clusterBinnedCellsEmpirical()

[Name,~] = uigetfile('*.mat','Select a pooledmap file with color-codded vectors');
            load(Name);

bins=[45,46,47,48,55,56,57,66,67,76,77,78,87];            
for i=1:size(pooledmap.clusterdata,2)
    if size(pooledmap.clusterdata{1,i}.UVStvecComp.VecUSt,1)>3
    ...
    ang=rad2deg(ang);
idx=kmeans(cos(ang),4);
% figure;
% hold on;
% circ_plot(ang(idx==1),'b');
% circ_plot(ang(idx==2),'g');
% circ_plot(ang(idx==3),'r');
% circ_plot(ang(idx==4),'m');

meanAng(i,1:4)=[mod(circ_mean(ang(idx==1)),2*pi),mod(circ_mean(ang(idx==2)),2*pi),...
        mod(circ_mean(ang(idx==3)),2*pi),mod(circ_mean(ang(idx==4)),2*pi)]
%     hold on
%     circ_plot(meanAng(i,1:4),'k'); 
